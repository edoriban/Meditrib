...
def read_suppliers(
    ...
    return db.query(models.Supplier).filter(models.Supplier.tenant_id == tenant_id).offset(skip).limit(limit).all()


@router.post("/", response_model=schemas.Supplier)
def create_supplier(
    supplier: schemas.SupplierCreate, db: Session = Depends(get_db), tenant_id: int = Depends(get_tenant_id)
):
...
def read_supplier(supplier_id: int, db: Session = Depends(get_db), tenant_id: int = Depends(get_tenant_id)):
    supplier = (
        db.query(models.Supplier)
        .filter(models.Supplier.id == supplier_id, models.Supplier.tenant_id == tenant_id)
        .first()
    )
    if not supplier:
...
def delete_supplier(supplier_id: int, db: Session = Depends(get_db), tenant_id: int = Depends(get_tenant_id)):
    supplier = (
        db.query(models.Supplier)
        .filter(models.Supplier.id == supplier_id, models.Supplier.tenant_id == tenant_id)
        .first()
