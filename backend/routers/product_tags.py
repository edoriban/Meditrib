...
def create_product_tag(
    tag: schemas.ProductTagCreate, db: Session = Depends(get_db), tenant_id: int = Depends(get_tenant_id)
):
...
def read_product_tag(tag_id: int, db: Session = Depends(get_db), tenant_id: int = Depends(get_tenant_id)):
...
def update_product_tag(
    tag_id: int, tag: schemas.ProductTagUpdate, db: Session = Depends(get_db), tenant_id: int = Depends(get_tenant_id)
):
    db_tag = crud_product_tags.get_product_tag(db, tag_id=tag_id)
    if db_tag is None or db_tag.tenant_id != tenant_id:
        raise HTTPException(status_code=404, detail="Tag not found")
...
def delete_product_tag(tag_id: int, db: Session = Depends(get_db), tenant_id: int = Depends(get_tenant_id)):
    db_tag = crud_product_tags.get_product_tag(db, tag_id=tag_id)
    if db_tag is None or db_tag.tenant_id != tenant_id:
        raise HTTPException(status_code=404, detail="Tag not found")
