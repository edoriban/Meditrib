...
def init_db():
    ...
    try:
        ...
        if not existing_admin:
            ...
            if update_needed:
                db.commit()
                print("[INIT_DB] Updated admin user with tenant_id/is_owner")

        # 4. Mark setup as completed for the default tenant
        setup_setting = (
            db.query(models.AppSettings)
            .filter(models.AppSettings.tenant_id == default_tenant.id, models.AppSettings.key == "setup_completed")
            .first()
        )

        if not setup_setting:
            setup_setting = models.AppSettings(tenant_id=default_tenant.id, key="setup_completed", value="true")
